...
export const createWireVisual = (wireData: WireData, wireDef: WireDefinition, size: number) => {
    return [
        k.pos(),
        k.rotate(0),
        k.anchor("center"),
        ...
        k.color(getWireColor(wireData, false)),
        k.scale(1),
        k.opacity(1),
        Tag_Wire_Visual,
    ]
...
export const createWire = (pos: Vec2, size: number, wireData: WireData, needBg: boolean, tags: string[] = [], parent: GameObj | null = null) => {
    ...
    wire.add(createWireVisual(wireData, wireDef, size));
